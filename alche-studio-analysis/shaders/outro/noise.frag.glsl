...
void main(void) {
    ...
    vec2 aspUv = vUv * vec2(uScreenAspectRatio, 1.0) - 0.5;
    vec2 nuv = aspUv * 0.5;

    // Warped noise sampling
    float n1 = noise3D(vec3(nuv + 1234.0, tn + 0.0));
    float n2 = noise3D(vec3(nuv + 5678.0, tn + 10.0));
    vec2 uv = aspUv * 0.6 + vec2(n1, n2) * 0.7;

    // 4-channel noise output
    vec4 col = vec4(0.0);
    col.x += noise3D(vec3(uv + 1.0, t + 0.0));
    col.y += noise3D(vec3(uv + 2.0, t + 1.0));
    col.z += noise3D(vec3(uv + 3.0, t + 2.0));
    col.w += noise3D(vec3(aspUv + noise3D(vec3(nuv + 1234.0, t * 0.)), t * 0.05 + 3.0));
    col = col * 0.5 + 0.5;
